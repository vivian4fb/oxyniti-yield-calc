...
## Amendment A9 and publication (2026-09-21, coordinating session)

- Owner inputs of 2026-09-21 applied (1-2 L/min for the 1.5 HP unit, 230 V x 3 A concentrator = 0.69 kW) with owner-authorised rough estimates for prices and maintenance (`plan/owner_inputs.json`, every value labelled). City (climate station) selection and the "Where does this species grow best?" comparison added; full payback now includes the concentrator and annual maintenance.
- Gate after A9: `node --test tests/*.test.mjs` 43/43; `python tests/compare.py` PASS on 29 scenarios (exit 0), `--self-test` PASS, `--doonly` PASS (max 0.000488 mg/L); placeholder scan clean; Chrome stderr clean of Uncaught/TypeError at 1280 px; screenshots `tests/screenshots/final4_http_1280x3000*.png`, `wp4b_*.png`.
- Default scenario (GIFT tilapia, Tiruchirappalli, 1 acre, 5,965/acre, 1.5 L/min, LT III-A): n_best 1, n_relief 9, ds 0.034, extra harvest 10-20 kg/crop, extra profit about -Rs 0.57 L/year, investment Rs 1.3 L, payback "No extra profit at these inputs". At 20 L/min per unit and 20,235/acre: one unit, +1,318 kg/crop, +Rs 0.62 L/year, payback 25 months (10 with a 60 % subsidy).
- Published: repository `github.com/vivian4fb/oxyniti-yield-calc` created 2026-09-21 (public, initial commit 801a23f), GitHub Pages enabled with the Actions workflow `.github/workflows/pages.yml` deploying `site/`. Live check recorded below once the first run completed.
- Live check 2026-09-21: Actions run 35613674866 completed with conclusion success; `https://vivian4fb.github.io/oxyniti-yield-calc/` returned HTTP 200 with the expected title, and `data/tn_districts.data.js` returned HTTP 200 (1,511,885 B). Screenshot `tests/screenshots/live_pages_1280x1000.png`.
- 2026-09-21 owner review: the Ukrainian-flag glyph in the map corner was Leaflet 1.9's default attribution prefix, not map content; `site/js/map.js` now sets the prefix to a plain "Leaflet" link. Verified in `tests/screenshots/final5_attribution_1280x1000.png`.
